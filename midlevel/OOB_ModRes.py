...
def OOB_ModRes(Test_frequency):
    ...
    sheet = FSV_file_write["OOB"] # load existing sheet named "ACP"
    ...
    sheet = FSV_file_write["OOB"] # load setup sheet in .xlsx to sheet
    ...
    FSV.write(f"INIT:CONT ON")# switch to continuous sweep prior to formal test
    time.sleep(1)
    SML.query('*OPC?')
    ...
    Reading_list.append(Dev_Reading)
