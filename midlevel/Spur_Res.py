...
def Spurious_response_immunity(Test_frequency):
    ...
    SSheet = SFile_write["Spurious_Response"] # load setup sheet in .xlsx to sheet
    ...
    RSheet = RFile_write["Spur_Res"] # load "ACP" sheet in .xlsx

    SINAD_data_str = CMS.query("SINAD:R?") # get initial SINAD value
    SINAD_data_num = re.findall(r'\d+\.\d+', SINAD_data_str)[0]
    print(f"Initial SINAD value for Spur_Res:{SINAD_data_num}")
    ...
    Spur_Res = float(SMB.query(f":POW? "))+3.5-(15.5-3.5)
    RSheet.cell(row = 2, column = 3, value = Spur_Res)
    ...
    #
    # SMB.query('*OPC?')
    indication = (SMB.query("*OPC?")).replace("1","Spur_Response test Completed")
    ...
    return {'Spur_Res':Spur_Res, 'Indication':indication}
