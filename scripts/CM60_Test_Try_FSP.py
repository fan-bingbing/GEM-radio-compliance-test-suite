...
Start_F = float(input("Input start frequency in Mhz or press CTRL+C to quit > "))
# Stop_F = int(input("Input stop frequency in Mhz or press CTRL+C to quit > "))
# steps = int((Stop_F-Start_F)*1000/12.5)

a = ((hex(int((Start_F*1e6)/(12.5*1e3)))[2]+hex(int((Start_F*1e6)/(12.5*1e3)))[3]))# calculate first HEX of input frequency
b = ((hex(int((Start_F*1e6)/(12.5*1e3)))[4]+hex(int((Start_F*1e6)/(12.5*1e3)))[5]))# calculate second HEX of input frequency

payload0[2] = int(a,16) # assign first DEC (transferred from HEX) to the third number of paylaod0
payload0[3] = int(b,16) # assign second DEC (transferred from HEX) to the fourth number of paylaod0

rm = visa.ResourceManager()
#FSV = rm.open_resource('TCPIP0::192.168.10.9::hislip0::INSTR') # Spec An
FSV = rm.open_resource('GPIB0::28::INSTR') # Spec An
#
...
# RSheet = RFile_write["try"] # load "ACP" sheet in .xlsx




# following code is to initialize FSV to complete the test
Centre_frequency = SSheet["B1"].value # get frequency from user input
Span_frequency = SSheet["B2"].value # get all parameters from Test_Setup.xlsx
RBW = SSheet["B3"].value #
VBW = SSheet["B4"].value #
...
FSV.write(f"SWE:TIME 680ms") # special setting for FSP
...
FSV.write(f"FREQ:SPAN {Span_frequency}kHz")# set span in the end as FSP response slower
time.sleep(3)
FSV.query("*OPC?")


CM60_Control.Radio_Con('com3', 9600, None, payload1)# set radio to 25W
CM60_Control.Radio_Con('com3', 9600, None, payload0)# set frequency
CM60_Control.Radio_Con('com3', 9600, None, payload2)# PTT on
...
print(f"Current Channel DEC:{Start_F}MHz")
