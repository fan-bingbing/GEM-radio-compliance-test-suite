...
file_name = input("input the filename (***.png) > ")
...
FSV.write(f"MMEM:NAME \'c:\\temp\\Dev_Screenshot.png\'")
# keep overwriting Dev_Screenshot.png in FSV C drive, note the usage of backslash \

FSV.write("HCOP:IMM")
# take the screenshot now
FSV.query("*OPC?")  # Wait for the screenshot to be saved

file_data = FSV.query_binary_values(f"MMEM:DATA? \'c:\\temp\\Dev_Screenshot.png\'", datatype='s',)[0]
