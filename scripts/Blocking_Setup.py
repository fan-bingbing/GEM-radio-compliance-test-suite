...
SSheet = SFile_write["Blocking"] # load "Blocking" sheet
...
RSheet = RFile_write["Blocking"] # load "Blocking" sheet
...
BLK = float(SMB.query(f":POW? "))-(45.5-6) # wanted signal expressed as dBuV emf
RSheet.cell(row = 2, column = 3, value = BLK)
print(f"BLK+:{BLK}")
# above code block to test BLK+

# prepare to test BLK-, reinitialize SMB
...
BLK = float(SMB.query(f":POW? "))-(45.5-6) # wanted signal expressed as dBuV emf
RSheet.cell(row = 2, column = 9, value = BLK)
print(f"BLK-:{BLK}")
#above code block to test BLK-
